...
$(function () {
  ...
  function fillList() {
    //Kod koji dodaje prirodno stanište vrste
    const source = document.getElementById("pokemon-list").innerHTML;
    const template = Handlebars.compile(source);
    const context = { pokemon: pokemons.slice(0, 20), tableClass: "table" }; //kod dodaje u tablicu brzinu rasta
  ...
  $.ajax("https://pokeapi.co/api/v2/pokemon-color/yellow", {
    ...
    error: function (jqXHR, textStatus, errorThrown) {
      //Funkcija koja se poziva na neuspješan kraj API poziva
      $("<div></div>")
        .insertAfter($("#pokemon-list"))
